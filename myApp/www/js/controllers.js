...
.controller('subjectsCtrl', function($scope, $http, ApiEndpoint) {
  ...
  $scope.addSubject = function() {
    $http.post(ApiEndpoint.url + '/assignatura', $scope.newSubject) // Enviamos la nueva asignatura (el nombre)
      .success(function(data) {
...
.controller('subjectsDetailCtrl', function($scope, $http, ApiEndpoint, $state, $ionicPopup) {
        ...
        //console.log("Alumnos inscritos en", $scope.subject.name, ":", estudiant);
          $http.get(ApiEndpoint.url + '/estudiant/' + estudiant)
            .success(function(data) {
              ...
              console.log("Nombre de alumnos en", $scope.subject.nom, ":", $scope.estudiantsList);
            })
            .error(function(data) {
              console.log('Error: ' + data);
            });
  ...
  $scope.addEstudiant = function() {
    ...
    $http.post(ApiEndpoint.url + '/assignatura/' + $scope.subjectID, $scope.updatedSubject)
      .success(function(data) {
        $scope.subject = data;
        $state.reload();
        //location.reload();
