...
angularRoutingApp.controller('assignaturaController', function($scope, $http) {
    ...
    // Obtenemos todos los datos de la base de datos filtrados por nombre
    $scope.FilterAssignaturaNom = function() {
        $http.post('/api/assignaturafilter', $scope.newFilter)
            .success(function(data) {
                $scope.newFilter = {}; // Borramos los datos del formulario
    ...
    $scope.filterAssignatureSemestre = function() {
        $http.post('/api/assignaturafilter2', $scope.newFilter)
            .success(function(data) {
                $scope.newFilter = {}; // Borramos los datos del formulario
                $scope.assignatures = data;
            })
    ...
    $scope.sinfilterAssignature = function() {
        $http.post('/api/sinfilterAssignature', $scope.newFilter)
            .success(function(data) {
                $scope.newFilter = {}; // Borramos los datos del formulario
                $scope.assignatures = data;
            })
